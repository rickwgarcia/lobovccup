...
export default function TrackCard({ track }: TrackCardProps) {
  ...
  return (
    <div
      className={`flex w-full min-w-0 max-w-[600px] flex-1 flex-col gap-6 rounded-card border border-brand-gray p-6 shadow-hard sm:p-9 ${
    ...
    >
      ...
      {isOpen && (
        <p
          id={panelId}
          className={`text-center font-grotesk text-body sm:text-left ${textColor}`}
        >
          {track.description}
        </p>
      )}
